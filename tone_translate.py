...
# =========================================================
# Render dictionary per language (ธรรมชาติในภาษาปลายทาง)
# =========================================================
TAG_RENDER: Dict[Lang, Dict[str, List[str]]] = {
    "th": {"BRO": ["มึง","แก"], "HUH": ["วะ","ว่ะ","นะ"], "LOL": ["555","ฮ่าๆ"],
           "KINDOF": ["เหมือนจะ","ค่อนข้าง"], "SARC": ["(ประชด)"]},
    "en": {"BRO": ["bro","dude","man"], "HUH": ["huh","right"], "LOL": ["lol","haha","lmao"],
           "KINDOF": ["kinda","sort of","low-key"], "SARC": ["(sarcasm)"]},
    "ja": {"BRO": ["お前","君"], "HUH": ["な","ね","よな"], "LOL": ["笑","草"],
           "KINDOF": ["なんか","ちょっと"], "SARC": ["（皮肉）"]},
    "zh": {"BRO": ["哥们","兄弟","老铁"], "HUH": ["吧","嘛"], "LOL": ["哈哈","笑死"],
           "KINDOF": ["有点","有那么点"], "SARC": ["（讽刺）"]},
    "ko": {"BRO": ["야","브로"], "HUH": ["냐","지"], "LOL": ["ㅋㅋ","ㅎㅎ"],
           "KINDOF": ["약간","좀"], "SARC": ["(비꼼)"]},
    "vi": {"BRO": ["mày","ông"], "HUH": ["hả","nhỉ"], "LOL": ["haha","kkk"],
           "KINDOF": ["kiểu như","hơi bị"], "SARC": ["(mỉa mai)"]},
    "fil": {"BRO": ["pre","pare","tol"], "HUH": ["no?","ha?"], "LOL": ["haha","lmao"],
            "KINDOF": ["parang","medyo"], "SARC": ["(sarkasmo)"]},
    "id": {"BRO": ["bro","bang"], "HUH": ["kan?","ya?"], "LOL": ["wkwk","haha"],
           "KINDOF": ["kayak","agak"], "SARC": ["(nyinyir)"]},
    "fr": {"BRO": ["mec","frère"], "HUH": ["hein","non?"], "LOL": ["mdr","haha"],
           "KINDOF": ["un peu","genre"], "SARC": ["(sarcasme)"]},
    "de": {"BRO": ["Alter","Digga"], "HUH": ["oder?","ne?"], "LOL": ["lol","haha"],
           "KINDOF": ["irgendwie","ein bisschen"], "SARC": ["(Sarkasmus)"]},
    "es": {"BRO": ["wey","tío"], "HUH": ["¿no?","¿eh?"], "LOL": ["jaja","xd"],
           "KINDOF": ["como que","medio"], "SARC": ["(sarcasmo)"]},
    "it": {"BRO": ["bro","amico"], "HUH": ["eh?","no?"], "LOL": ["ahaha","lol"],
           "KINDOF": ["tipo","un po'"], "SARC": ["(sarcasmo)"]},
    "pt": {"BRO": ["mano","cara"], "HUH": ["né?","hein?"], "LOL": ["kkk","haha"],
           "KINDOF": ["meio que","um pouco"], "SARC": ["(sarcasmo)"]},
    "pl": {"BRO": ["stary","ziomek"], "HUH": ["no nie?","co?"], "LOL": ["xD","haha"],
           "KINDOF": ["trochę","tak jakby"], "SARC": ["(sarkazm)"]},
    "uk": {"BRO": ["друже","чувак"], "HUH": ["га?","так?"], "LOL": ["лол","хаха"],
           "KINDOF": ["ніби","трохи"], "SARC": ["(сарказм)"]},
    "ru": {"BRO": ["чувак","бро"], "HUH": ["а?","да?"], "LOL": ["лол","ахаха"],
           "KINDOF": ["типа","слегка"], "SARC": ["(сарказм)"]},
    "hi": {"BRO": ["यार","भाई"], "HUH": ["ना","क्या"], "LOL": ["हाहा","lol"],
           "KINDOF": ["थोड़ा","किस्म का"], "SARC": ["(तंज)"]},
    "km": {"BRO": ["បង"], "HUH": ["ដែរ?","ម៉េច?"], "LOL": ["ហា​ហា"],
           "KINDOF": ["ប្រហែល","តិចតួច"], "SARC": ["(សើចចំ)"]},
    "my": {"BRO": ["ရေ"], "HUH": ["လား","နော်"], "LOL": ["ဟားဟား"],
           "KINDOF": ["အနည်းငယ်"], "SARC": ["(သံယောဇဥ်)"]},
    "ar": {"BRO": ["يا زلمة","يا صاح"], "HUH": ["ها؟","مش كده؟"], "LOL": ["هههه","لول"],
           "KINDOF": ["شوية","كده"], "SARC": ["(سخرية)"]},
}
...
# ถ้าไม่ได้อนุญาต <HUH> ให้ลบคำลงท้ายประเภทฟิลเลอร์ที่หลุดมาจริงๆ
TRAILING_FILLERS: Dict[Lang, str] = {
    "en": r"(?:\s*,?\s*(?:huh|right|eh))[\.\?!]*\s*$",
    "th": r"(?:\s*(?:วะ|ว่ะ|นะ|ปะ))\s*$",
    "ja": r"(?:[なね](?:よ)?)[？?]?\s*$",
    "zh": r"(?:吧|嘛)\s*$",
    "ko": r"(?:냐\?)\s*$",
    "es": r"(?:¿no\?)\s*$",
...
TRAILING_FILLERS["fil"] = r"(?:\s*(?:no|ha)\?)\s*$"
TRAILING_FILLERS["fil-PH"] = TRAILING_FILLERS["fil"]
TRAILING_FILLERS["tl"] = TRAILING_FILLERS["fil"]
TRAILING_FILLERS["id"] = r"(?:\s*(?:kan|ya)\?)\s*$"

def _strip_trailing_fillers_if_no_huh_tag(text: str, tgt: Lang, allowed: set[str]) -> str:
    if "HUH" in allowed or not text:
        return text
    pat = TRAILING_FILLERS.get(tgt)
    if not pat:
        return text
...
# ❗ FIXED: ต้องมีตัวลงท้ายจริง ๆ เท่านั้นถึงจะติด <HUH>
_TAGS = {
    ...
    # บรรทัดนี้แหละที่แก้: ห้าม pattern ที่แมตช์สตริงว่าง
    "HUH_END": r"(?:(?:วะ|ว่ะ|นะ|ปะ|หรือไง|หรือเปล่า)\s*(?:\?|$)|\b(?:huh|right|eh)\b(?:[.!?]|$)|¿no\?|né\?|[なね](?:よ)?[？?]?$|(?:吧|嘛)$|(?:吗|嗎)$|냐\?$)",
...
def pre_annotate(text: str, src_lang: Lang) -> Tuple[str, ToneHints]:
    ...
    if re.search(_TAGS["BRO"], t, flags=re.I):
        t = f"<BRO>{t}"; hints.has_slang = True
    if re.search(_TAGS["LOL"], t, flags=re.I):
        t = f"<LOL>{t}"; hints.has_slang = True
    if re.search(_TAGS["KINDOF"], t, flags=re.I):
        t = f"<KINDOF>{t}"; hints.has_slang = True
    if re.search(_TAGS["HUH_END"], t, flags=re.I):
        t = f"{t} <HUH>"; hints.has_slang = True
    if re.search(_TAGS["SARC"], t, flags=re.I):
        t = f"<SARC>{t}"; hints.sarcasm = True
...
def build_natural_prompt(src: Lang, tgt: Lang, hints: ToneHints) -> str:
    ...
    intensity_line = {"soft": "Hedge more if casual. ", "normal": "", "hard": "Allow strong slang/insults if natural. "}[hints.intensity]
    return (
        ...
        "5) Only use discourse markers like 'huh/right/eh/ね/吧/냐/¿no?' if <HUH> is present in the input; otherwise avoid them.\n"
...
async def smart_translate(
    ...
    *,
    ...
    allowed_tags = _collect_tags(annotated)
    ...
    # 1) ห้ามแท็กที่ไม่ได้อนุญาต
    raw = _strip_unallowed_tags(raw, allowed_tags)
    # 2) ถ้าไม่ได้ติด <HUH> ไว้แต่ LLM แถมฟิลเลอร์มา → ตัดออก
    raw = _strip_trailing_fillers_if_no_huh_tag(raw, tgt, allowed_tags)
