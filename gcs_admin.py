...
async def gcs_delete_all_objects(bucket: str, prefix: Optional[str] = None) -> int:
    """ลบ objects ทั้งหมดในบัคเก็ต (ตาม prefix ถ้ามี) คืนจำนวนที่ลบได้"""
    deleted = 0
    page = None
    while True:
        ...
        for obj in items:
            try:
                await _delete_object(bucket, obj.get("name", ""))
                deleted += 1
            except httpx.HTTPStatusError as e:
                # ถ้าลบไม่ได้ก็ข้าม แต่ยังดำเนินต่อ
                # (เช่น object ถูกลบไปแล้ว/สิทธิ์ไม่พอ)
                pass
...
async def gcs_delete_bucket(bucket: str, *, force: bool = False, prefix: Optional[str] = None) -> Tuple[bool, str]:
    """
    ลบบัคเก็ต:
      - ถ้า force=True จะลบ objects (optionally filter ด้วย prefix) ก่อนอีกรอบ
      - ต้องใช้สิทธิ์: storage.objects.delete + storage.buckets.delete
    คืน (ok, message)
    """
    try:
        if force:
