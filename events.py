...
def _ensure_alts_for_code_switch(base_lang_code: str, alt_iso: list[str] | None) -> list[str]:
    ...
    # ไทย/เขมร/พม่า ปนอังกฤษบ่อย → เติม en
    if base_family in {"th", "km", "my"} and "en" not in fams:
        alts = ["en"] + alts
    # สำหรับ km/my ให้เติม th ไว้เผื่อด้วย (เจอ code-switch กับไทยบ่อย)
        ...
        if channel_cfg == "multi" and message.attachments:
            ...
            if audio_attachments:
                ...
                async def _run_stt_with_lang(interaction, base_lang_code: str):
                    ...
                    """
                    # ✅ NEW: normalize lang ที่ผู้ใช้เลือกเป็น BCP-47 เสมอ (เช่น km -> km-KH)
                    ...
                    try:
                        ...
                        # ใส่ en สำหรับ th/km/my และจำกัด 3 ตัว
                        ...
                        # รอบ 2: ถ้าว่าง หรือ "ภาษาที่ตรวจได้" ไม่ตรง family กับที่เลือก → ลองใส่ alt
                ...
                panel = STTLanguagePanel(
                    ...
                    flags=FLAGS,
                    major_langs=["th", "en", "ja", "km"],  # ✅ เพิ่ม km ให้เลือกสะดวก
                    major_primary="th",
                )
